...
do
    model=${args[i]}
    num_gpu=${args[i+1]}
    echo "python model_generate.py --num_shots 4 --output ~/model_evals --dataset math --override --detect_chat --model $model --tensor_parallel_size $num_gpu"
    python model_generate.py --num_shots 4 --output ~/model_evals --dataset math --override --detect_chat --model $model --tensor_parallel_size $num_gpu
done